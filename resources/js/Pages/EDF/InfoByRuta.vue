...
const filteredClients = computed(() => {
    let clients = props.clients.filter(client => {
        ...
        return (matchesCOD_CLIENTE || matchesCLIENTE) && matchesFilter && matchesDay;
    });
...
const filteredNegociados = computed(() => {
    return props.clients.filter(client => client.NEGOCIADO === 'NEGOCIADO' && (client.FREC_VISITA.includes(selectedDay.value) || selectedDay.value === '')).length;
});

const filteredNoNegociados = computed(() => {
    return props.clients.filter(client => client.NEGOCIADO === 'PENDIENTE' && (client.FREC_VISITA.includes(selectedDay.value) || selectedDay.value === '')).length;
});
</script>
    ...
    <GuestLayout :title="`Dashboard - ${props.ruta}`">
        <template #header>
            <div class="flex justify-between items-center">
                ...
                <select v-model="selectedDay" class="form-select block w-1/4 mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-2 overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="flex flex-col gap-4">
                        <div class="bg-white shadow-md rounded-lg overflow-hidden flex" v-for="client in filteredClients">
                            <div class="flex-1 border-r border-gray-200">
                                <div class="p-4">
                                    <div class="text-sm font-semibold"><i class="fa-solid fa-user"></i> {{ client.COD_CLIENTE }} - {{ client.CLIENTE }}</div>
                                    <div class="text-sm"><i class="fa-solid fa-location-dot"></i> {{ client.DIRECCIÓN }}</div>
                                    <div class="mt-2">
                            ...
                            <div v-if="client.NEGOCIADO == 'PENDIENTE'" class="flex-1 text-left">
                                <div class="p-4 text-left">
                                    ...
                                    <a :href="`https://wa.me/?text=Quiero%20negociar%20EDF%20para%20el%20cliente%20${client.COD_CLIENTE}`" target="_blank" class="ml-auto bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded-md">
                            ...
                            <div v-else class="flex-1 text-left">
                                <div class="p-4 text-left">
                                    <div class="text-sm font-medium text-gray-500"><i class="fa-solid fa-check-to-slot mr-2"></i>Haz Negociado</div>
                                    <div class="flex items-center mt-2">
                                        <div class="text-sm font-semibold">EDF | {{ client.PUERTAS_A_NEGOCIAR }} Pts | {{client.CONDICION}}</div>
                                    </div>
                                    <button class="ml-auto bg-green-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded-md">
